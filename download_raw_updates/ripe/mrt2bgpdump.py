...
class BgpDump:
    ...
    def print_line(self, prefix, next_hop, msg_len):
        ...
        if self.flag == 'B' or self.flag == 'A':
            self.output.write('%s|%s|%s|%s|%s|%s|%s|%s' % (
                d, self.flag, self.peer_ip, self.peer_as, prefix,
                self.merge_as_path(), self.origin, msg_len))
	    #self.output.write('%s|%s|%s|%s|%s|%s|%s|%s' % (
            ...
            self.output.write('%s|%s|%s|%s|%s|%s\n' % (
                d, self.flag, self.peer_ip, self.peer_as,
                prefix, msg_len))
	    #self.output.write('%s|%s|%s|%s|%s|%s\n' % (
    ...
    def print_routes(self,msg_len):
        for withdrawn in self.withdrawn:
            if self.type == 'BGP4MP':
                self.flag = 'W'
            self.print_line(withdrawn, '',msg_len)
        for nlri in self.nlri:
            if self.type == 'BGP4MP':
                self.flag = 'A'
            for next_hop in self.next_hop:
                self.print_line(nlri, next_hop,msg_len)
    ...
    def bgp4mp(self, m, count):
		...
		self.print_line([], '', '0')
            ...
            self.print_routes(m.bgp.msg.len)
